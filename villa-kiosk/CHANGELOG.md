...
2.144.0 implemented the redesign from `VESTA-DESIGN.md` and the `assets/` folder. The handoff also shipped two spec documents — a Brand & UI Guidelines and a live UI Kit — and those were never opened. They carry rules the instruction file only alludes to, so an audit against them found nine genuine gaps. The design tokens themselves were already exact: a mechanical diff of all 140 values in `assets/vesta-tokens.json` against the `:root` / `[data-theme]` blocks matches with zero differences, which is why nothing below touches a colour.

**Icon sizes were inverted.** The guidelines' iconography section is specific: 24px in the top bar, 20px in badges and bottom-bar tiles, 18px inside chips and buttons, 17px inside segmented controls, and nothing smaller than 16px anywhere. The app had the two most visible cases exactly backwards — the top bar rendering at 18px and the bottom-bar tiles at 24px — plus 43 icons scattered below the 16px floor (a 12px glyph in a Facility list row, a 13px one in a Cockpit tile). All corrected. The two exceptions kept deliberately: the profile chip's sign-out glyph and every row in the overflow dropdown stay at 18px, because both are chips/menu buttons rather than top-bar icons.

**The badge state table is now honoured in full.** Three of its four rows were already right; the idle row was not. The guidelines give even a resting badge a ring — 1px `--hairline` — and `categorySurface()` was returning `null` there, with `badgeIcons.ts` compensating via a hardcoded `rgba(0,0,0,0.28)` edge drawn under every state. That hardcoded edge is gone: idle draws the real hairline, active/alerting draw 1.5px in the state colour, unavailable draws 1.5px dashed amber, all from the same declared surface. The active/alerting tint moved from 18% to the specified 14%. Badge geometry went from 40×40 to the stated **44×44 minimum**, which is also this app's own `--touch-min` — a wall tablet operated standing up by someone who has never seen it before should not have a sub-44px tap target, and the 12px corner radius now falls out of the existing corner fraction at that size (44 × 0.28 = 12.3) rather than needing its own constant.

**One panel material, one radius.** The guidelines describe a single chrome material — `--bg-panel`, `blur(20px) saturate(1.1)`, a 1px hairline and an **18px** radius — shared by the top bar, the floor switch and the bottom bar. The bottom bar already had it; the three top-bar containers were still on the 14px squircle. They now take `--panel-radius`, with the squircle reserved for the controls sitting *on* the panel, so container and control are deliberately two shapes instead of accidentally one.

That change forced a second one worth recording, because it is the kind of arithmetic that silently breaks later: `.hud-group` sets `--hud-pill-h` as an *explicit* height, so the token has to equal its own contents exactly. Raising the category chips from 38px to the specified 40px made the true height 40 + 2×4px padding + 2×1px border = 50, against a token still reading 48 — a 2px overflow. `--hud-pill-h` is now 50 and carries that sum in a comment. The ≤640px compact query's own 42 is the same arithmetic at a 32px chip and needed no change.

**Two smaller items from the same documents:** the bottom-bar label is display type (11px Jost caps) and was inheriting Public Sans, and the alert counters were 14px against a stated 16px.

**A gap neither document mentions, found while checking them.** `EntityVisuals`'s `GUI_FONT_FAMILY` — the font every Babylon GUI `TextBlock` in the scene must set explicitly, or it silently falls back to Arial — was still the pre-rebrand system stack. Room-cluster chips and badge value pills were therefore rendering in the platform's own UI font while every DOM surface beside them had moved to Public Sans. Public Sans now leads that stack; the system fonts stay behind it as the pre-load fallback, since a canvas can only use a webfont once it has actually loaded.

**One rule from the guidelines is deliberately NOT followed**, and this is the important note for anyone reconciling the two later. The badge table specifies `--bg-input` as the idle and unavailable fill. That is precisely what shipped in 2.144.0 and had to be reverted in 2.145.0: `--bg-input` is 5% black, which reads correctly as a DOM chip on a panel and is effectively invisible baked into a canvas texture floating over the 3D villa — reported from a screenshot, with resting devices showing no badge at all. The fill stays composited onto the opaque panel colour. The visual result over that backdrop is identical to what the guidelines intend; it simply survives being painted onto a transparent canvas, which the document's author had no way to know was one of the two consumers of that value.
...
A complete design handoff (`VESTA-DESIGN.md`, drawn from a "VESTA — AI Estate Manager" brand deck) replaced the interim reskin from 2.139.0 wholesale — new colour tokens, self-hosted display type, a redesigned badge/state language, and a from-scratch PWA icon set. Landed in the order the handoff specified, since each stage builds on the last.

**Design tokens + a third theme.** `styles.css`'s light/dark blocks were replaced with the new palette (`--cat-*` category hues, `--accent-soft`, the `--space-*`/`--elev-*`/`--dur-*` scales), and a `night` theme was added — dimmer and warmer than plain dark, for a tablet mounted in a bedroom. Auto used to be a pure CSS `prefers-color-scheme` media query; picking night over dark needs the real sun position, which CSS can't express, so `ConfigContext`'s theme effect now resolves "auto" in JS (`utils/themeTime.ts`) using the same NOAA-style altitude approximation `SunController` already uses for the 3D scene's lighting, driven by the villa's own HA-sourced lat/lng — re-resolved every 5 minutes so a kiosk left running overnight crosses into night on its own, and re-checked on every OS light/dark change. Jost (headlines/eyebrows) and Public Sans (everything else) are self-hosted in `public/fonts/` — never a Google Fonts link, since the kiosk's whole reason for existing is working with no internet at all.

**Neutral-by-default badges.** Every badge and bottom-bar tile used to carry a saturated category gradient permanently, so a villa at rest looked like a villa in alarm. `config/EntityCategories.categorySurface(category, state)` is now the one function every badge surface — 2D chips, the legend, bottom-bar tiles, and the canvas-baked 3D squircle (`babylon/badgeIcons.ts`) — calls for its fill/glyph/ring: plain neutral until a device is active or alerting, category-hued only then, a dashed amber ring for unavailable (previously just a flat 40%-alpha wash on the always-on gradient). Reworking this surfaced three near-duplicate "is this device on" implementations (the map badge's `badgeKind`, the panel header icon, the device-group list); they now share one classifier (`utils/deviceActivity.ts`) instead of drifting independently. `colors.ts`'s standalone Babylon-side reds/ambers (`ALERT_RED`, `UNAVAILABLE_AMBER`, `AVAILABLE_GREEN_HEX`) were updated to the new palette's values, and every lucide icon in the app now renders at one standard 1.5px stroke weight via a single CSS override of the presentation attribute lucide sets, rather than touching every one of its call sites individually.

**HUD + modal chrome.** Top-bar/bottom-bar glass moved to the new blur/elevation values; the category filter row switched from a per-category gradient chip to a shared accent-soft tint — filters toggle visibility, they don't need their own hue, the map badges already carry that signal. Segmented controls (the theme picker, Day/Auto/Night) no longer show their selected option as a solid green block — three of those per screen used to compete with the app's real accent moments — now a soft lifted chip instead. Modal titles, section headers and sliders picked up the new type scale and token values; bottom-bar tiles lost their individual card borders in favour of hairline dividers between them, reading as one bar instead of a row of separate chips.

**PWA identity: renamed to VESTA.** The provided manifest renamed the app from "Vesta Kiosk" to "VESTA — AI Estate Manager" (confirmed with the user first, since a home-screen app rename is a bigger deal than a colour swap) — carried through every user-facing surface: the manifest, `index.html`'s title/meta, the HUD's default title fallback, error/diagnostic reports, the Facility report footer, and the HA add-on's own `name`/`panel_title` in `config.yaml`. Internal identifiers that intentionally don't track branding — the add-on `slug`, the GHCR image path, `localStorage` keys, service-worker cache names — were left alone, per the existing rule that changing any of those orphans a real install's data on upgrade. New icon/splash assets (favicon, PWA icons, Apple touch-startup images for both iPad postures and iPhone portrait) replaced the old navy set; a manifest `shortcuts` entry for deep-linking to specific panels was dropped rather than shipped, since the app has no `?panel=` routing to back it. The service-worker cache bumped `v8` → `v9` so returning clients evict the old icon set instead of serving it stale.
...
**Modal title.** `.modal h2` — the shared rule every dialog's `<h2>` matches (Settings, Config Editor, Facility, Cockpit, Legend, Rooms, both Facility sub-modals) — had never actually set a `font-size`, so it fell through to the browser's own unstyled default instead of the type scale's `--text-xl`, which is literally documented as "panel + modal headings" two lines above where it should have been used. The entity panel's own title (`.panel-header .title h2`) already had a deliberately smaller, explicit size (18px, so a long device name gets more room before truncating). Both rules always shared the same `var(--font-display)` family, so this was never actually "two different fonts" — but an accidental, UA-dependent size sitting next to a deliberately-tuned one reads exactly like it was. Fixed by giving `.modal h2` its own explicit `var(--text-xl)` (20px), so every dialog now has one consistent, intentional title size, and the entity panel's 2px-smaller size is a clearly-scoped override on top of that shared base rather than an independent, driftable value.

**Stale icons.** The service worker caches same-origin static assets cache-first with a background refresh (`public/sw.js`) — correct for content-hashed build output, but the favicon/manifest icons swapped in the rebrand keep the same unhashed filenames release to release, so a browser that had already cached the old ones kept serving them (one load behind the real fix, since the background refresh only updates the cache for *next* time). Bumped the service-worker cache version (`v7` → `v8`) so its `activate` handler evicts the old cache wholesale and every client re-fetches the current icon set on next open, instead of waiting on the natural one-load staleness cycle.

**First-person freeze, round two.** The previous release's submesh-octree fix (below) confirmed it fixed "Clickable Glow" but did not resolve the movement freeze on its own — a submesh octree only prunes a raycast if the target mesh actually has enough submeshes to spatially divide, which this villa's fused structural mesh may not. Added the second, unconditional half of the fix: `CameraController.followFloor()`'s downward raycast — the one that runs every single rendered frame while walking — is now time-throttled to ~11 Hz (`FLOOR_PROBE_INTERVAL_MS`) instead of once per frame, reusing the last answer between probes. This caps the actual COST of the expensive operation directly, independent of whether the octree helps, the same "measure/cap by wall-clock time, not frame count" approach `frameFactor()` already uses elsewhere in this file. The smoothing that consumes the raycast's answer still runs every frame, so motion stays exactly as continuous — only how often the expensive question gets re-asked changes, and normal walking speed doesn't cover enough ground in 90ms for a slightly-stale floor answer to be visible.
