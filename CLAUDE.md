...
- **Shared rules live in one place, and these are they:** `roomKey()`/`sameRoom()` (`config/roomKey.ts`) for every room-name comparison — case- and whitespace-insensitive, never store the key; `useEntityLabel()` for "what do we call this device"; `useLongPress`/`useSuperadminDelete` for press-and-hold; `useStoreRefresh` + `useSyncReporter` + `utils/keyedSync.ts` for the two server-backed stores; `selectableDeviceIds` for "what counts as a device"; `--field-label-gap`/`--field-label-size` for a labelled form field's rhythm. Reach for these before writing the rule again.
