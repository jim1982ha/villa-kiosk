...
- **Shared rules live in one place, and these are they:** `roomKey()` (`config/roomKey.ts`) for every room-name comparison — case- and whitespace-insensitive, never store the key, and normalise the fixed side ONCE outside a loop rather than per-iteration; `useEntityLabel()` for "what do we call this device"; `useLongPress`/`useSuperadminDelete` for press-and-hold; `useStoreRefresh` + `useSyncReporter` + `utils/keyedSync.ts` for the two server-backed stores; `useModalA11y` for a dialog's focus trap / Escape / focus restore; `usePendingAck` (acknowledge, never predict) and `useOptimisticToggle` (predict, DOM switches only — never scene state) for control feedback; `utils/haptics.ts` for touch feedback; `atomic_write`/`atomic_write_async` (`supervisor-proxy.py`) for every file write under `/data`; `selectableDeviceIds` for "what counts as a device"; the `--text-*` scale for every font size; `--field-label-gap`/`--field-label-size` for a labelled form field's rhythm. Reach for these before writing the rule again.
