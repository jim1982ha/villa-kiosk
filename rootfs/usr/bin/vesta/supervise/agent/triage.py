...
from vesta.supervise.agent import naming as naming_mod
...
#: ⚠️ MOVED TO `agent/naming.py` WITH THE MATCHER IT GOVERNS (2.951.0), and
#: re-exported because `test_subject_identity` and the note below both name it.
REVERSE_MIN_SHARE: float = naming_mod.REVERSE_MIN_SHARE
...
def _identify(items: Sequence[Escalation], refs: Any) -> None:
    ...
    for item in items:
        ...
        if not subject:
            continue
        hits = naming_mod.devices_named(subject, known)
        if hits:
