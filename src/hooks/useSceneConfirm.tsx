...
//
// ⚠️ ONE HOOK BECAUSE THERE ARE TWO SURFACES. Scenes can be run from the
// summary bar's popover and from a room panel's "Scenes for this room" row,
// and those two must not answer the same question differently. The hook owns
// the question, the haptic and the call, so a third surface gets all three by
// using it and cannot get two of them by copying.
//
...
//
// ⚠️ NO `confirmIcon`/`cancelIcon` HERE, UNLIKE THE DEV BRANCH'S COPY. This
// branch's `AskDialog` has no icon props — they were added later — and passing
// them would not typecheck. Backported behaviour, not backported source.
