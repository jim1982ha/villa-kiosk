        ...
        <div key={i} className="editable-row-card">
          <div className="editable-row" style={{ marginTop: 8 }}>
            <div className="editable-row-fields editable-row-tight">
              ...
              {/* ⚠️ THE LABEL IS BACK, SHORTENED — owner, 2026-09-18. It was
                  removed in 2.975.0 as redundant and restored one release
                  later: what was actually wrong was its LENGTH, not its
                  presence. "Devices — briefings are sent here" wrapped and
                  repeated on every row; "Notif destination" says the same
                  thing in the width the column has. Recorded rather than
                  silently re-added, so the rule above ("every field carries a
                  visible label") reads as unbroken — which it now is. */}
              <label className="people-field">
                <span>Notif destination</span>
                <RecipientButton
                  targets={row.targets ?? []}
                  available={targets}
                  open={open === i}
