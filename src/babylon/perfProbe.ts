...
function buildConditions(scene: Scene, guiLayers: readonly { rootContainer: { isVisible: boolean } }[]): Condition[] {
  return [
    ...
    {
      // The lamp glow on the lightmapped structure (lampGlow.ts): a loop over
      // the lamps that are on, per structure pixel. Its lamp count is a
      // UNIFORM, so unlike the row above this ablates cleanly — no recompile.
      // Skipped when no lamp is on, which is when it costs nothing anyway.
      name: "no lamp glow",
      apply: () => {
