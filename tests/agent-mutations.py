...
MUTATIONS = [
    ...
    ("gateway: a reply with no summary reports an empty property",
     "agent/gateway.py",
     '        if not isinstance(summary, dict) or "total_entities" not in summary:',
     "        if False:"),
    ("gateway: the count comes from a truncated sample, not the total",
     "agent/gateway.py", '            return int(summary["total_entities"])',
     '            return len(summary.get("domain_stats", {}))'),
    ...
    ("gateway: the MCP session id is never echoed back",
     "agent/gateway.py", '            headers["Mcp-Session-Id"] = self._session_id',
     "            pass"),
    ("gateway: the handshake is never confirmed",
     "agent/gateway.py", '            await self._rpc("notifications/initialized", {}, notify=True)',
     "            pass"),
    ("gateway: a stale session is replayed to a new connection",
     "agent/gateway.py", "            self._session_id = None\n            await self._rpc(\"initialize\"",
     "            await self._rpc(\"initialize\""),
    ("gateway: an empty body is reported as a parse error again",
     "agent/gateway.py", '        if body is None:\n            raise GatewayError(', '        if False:\n            raise GatewayError('),
    ("gateway: every request reuses id 1",
     "agent/gateway.py", '            request["id"] = self._next_id\n            self._next_id += 1',
     '            request["id"] = 1'),
